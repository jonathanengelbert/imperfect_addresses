...
for i in range(1, ws.max_row + 1):
        ...
        #-----------------------------------------------------------------------------------------------------------------------
            ...
            # TRANSFORMS SPELLED OUT NUMBERS

                spelled_out_number_dict = {"first": "1st",
                                           "one": "1st",
                                           "second": "2nd",
                                           "third": "3rd",
                                           "fourth": "4th",
                ...
                for input, case in spelled_out_number_dict.items():
                    if (input in value):
                        if ("sixth street" and "sixth st" not in value):
                          value = value.replace(input, case)


            #LOOKS FOR ADDRESSES THAT DO NOT HAVE STREET TYPE AND TRIES TO
            # MATCH THEM AGAINST EAS DB

                match = re.findall(
                    r"\save\b|blvd\b|cir\b|\bdr\b|expy\b|hwy\b|hill\b|ln\b"
                    r"|park\b|pl\b|plz\b|pier\b|rd\b|stps\b|\sst\b|sq\b|ter\b|tunl\b|way\b",
                    value)

                if not match:

                    for address, address_type in eas_library.address_library.items():
                        if value in address:
